...
title: AES-ECB 加密
difficulty: hard
tags: ["symmetric", "aes", "block-cipher", "encrypt"]
algorithm: aes_ecb_encrypt
testcase_count: 5
params:
  plaintext_hex:
    type: hex_string
  ...
  key_hex:
    ...
    max_len: 32
generator: |
  # Pure-Python AES-128 (no external libraries required)
  ...
  pt_hex = input()
  key_hex = input()
  ct = aes_ecb(bytes.fromhex(pt_hex), bytes.fromhex(key_hex))
  print(ct.hex())
starter_code: |
...
AES-ECB（Advanced Encryption Standard - Electronic Codebook）是一種對稱區塊加密模式，每個 16 位元組區塊獨立加密。

## 輸入說明

```
明文（32 個十六進位字元 = 16 位元組）
...
```
密文（十六進位字串，含 PKCS#7 填充，共 64 個十六進位字元）
```

## 說明

1. 將十六進位字串解碼為位元組
2. 使用 PKCS#7 填充至 16 的倍數（16 位元組輸入填充後為 32 位元組）
3. 進行 AES-128 ECB 加密
4. 輸出為小寫十六進位字串

**提示：** 使用 `pycryptodome` 的 `AES` 模組。
